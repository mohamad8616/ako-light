...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-30 bg-black/80 transition-opacity delay-1000 duration-1000 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Popup
        ...
        data-side={side}
        className={cn(
          "fixed z-30 flex flex-col bg-popover bg-clip-padding text-xs/relaxed text-popover-foreground shadow-lg",
          !motionProps &&
