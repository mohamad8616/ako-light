...
// ---------------------------------------------------------------------------
// Module-scope constants — allocated once, never on each render.
// ---------------------------------------------------------------------------

const ICON_CLASS = "h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-4 md:w-4 lg:h-4 lg:w-4";

const SOCIAL_BTN_CLASS =
  "group flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#292929] text-[#eeeeee] transition-all duration-300 hover:border-[#666] hover:bg-[#181818] sm:h-10 sm:w-10 md:h-12 md:w-12 lg:h-15 lg:w-15";

const SOCIAL_WRAPPER_CLASS =
  "mt-7 flex w-full flex-wrap items-center justify-between md:mt-0 md:w-auto md:justify-end md:gap-3";

// Shared <svg> props — keeps each icon component a tiny render.
const SVG_BASE = {
  viewBox: "0 0 24 24",
  "aria-hidden": true,
  focusable: false,
...
// ---------------------------------------------------------------------------
// Memoized icon components. `memo` means an icon only re-renders when its
// own props change — and here there are no props, so each icon renders once
...
const WhatsAppIcon = memo(function WhatsAppIcon() {
  return (
    <svg {...SVG_BASE} fill="currentColor" className={ICON_CLASS}>
...
// Map allocated once at module load — no per-render object literal.
const SOCIAL_ICONS: Record<SocialIconKey, React.ComponentType> = {
...
export default function Footer() {
  ...
  return (
    <footer className="bg-background text-background-secondary">
      <HomepageSection className="ssm:py-10 flex min-h-50 flex-col py-8 md:flex-row md:items-center md:justify-between md:gap-12 md:py-12">
        {/* Top / Left */}
