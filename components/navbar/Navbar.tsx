...
export default function Navbar() {
  ...
  // Hide the whole navbar while the hero video is playing — it would
  // otherwise sit over the top-right close button (transparent full-width
  // bar) and swallow the click that closes the player.
  ...
  return (
    <>
      <motion.header
        initial={{ y: 0 }}
        animate={{ y: hidden || isVideoPlaying ? "-100%" : 0 }}
        transition={{
          duration: 0.5,
          ease: [0.22, 1, 0.36, 1],
        }}
        className={`fixed inset-x-0 top-0 ${menuOpen ? "z-[1000]" : "z-50"} ${overlayOpen || isVideoPlaying ? "pointer-events-none" : ""}`}
      >
